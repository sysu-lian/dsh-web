...
/** Extract a readable one-line summary from tool arguments (command/path/url). */
...
/** Official ReasoningRow: collapsed-by-default Think row (QWLzlG_* classes,
 *  verbatim DOM from dsh-client-ui-conversation). */
function OfficialReasoningRow({ text, pending }: { text: string; pending: boolean }) {
...
/** Official GenericCommandCard-style tool row: collapsed bash/command call
 *  inside the thinking chain (_Xvjua_* classes, verbatim DOM). No copy. */
function OfficialToolRow({ tool, failed }: { tool: ToolCallInfo; failed?: boolean }) {
  const [open, setOpen] = useState(false)
